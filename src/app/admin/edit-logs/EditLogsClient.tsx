  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        {logs.map((log, index) => (
          <div key={log.id} className="nox-card animate-fade-up" style={{ animationDelay: `${index * 0.03}s` }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem", alignItems: "flex-start" }}>
              ...
              <button className="nox-btn nox-btn-ghost nox-btn-sm" type="button" onClick={() => deleteLog(log)} style={{ color: "#f87171" }}>
                <Trash2 size={15} />
                Delete
              </button>
            </div>
            ...
            <div style={{ marginTop: "0.875rem", display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
              {log.changes.length > 0 ? log.changes.map((change) => (
                ...
                <span style={{ color: "var(--nox-text-muted)", fontSize: "0.75rem" }}>No line value changed.</span>
              )}
            </div>
