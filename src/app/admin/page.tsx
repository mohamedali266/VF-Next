...
export default async function AdminDashboard() {
  ...
  const stats = [
    { icon: "Users", label: "Active Users", value: totalUsers, color: "var(--nox-red-light)" },
    ...
    { icon: "HC", label: "Health Checks", value: todayChecks, color: "var(--nox-success)" },
  ];
  ...
  const actions = [
    ...
    { icon: "Branches", title: "Branches", sub: "Create branches and review teams", href: "/admin/branches", color: "rgba(139,92,246,0.1)", border: "rgba(139,92,246,0.25)" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "0.75rem" }}>
        {stats.map((stat, i) => (
          <div key={stat.label} className={`nox-card animate-fade-up animate-fade-up-delay-${i + 1}`} style={{ textAlign: "center", padding: "1.25rem 0.75rem" }}>
            <div style={{ fontSize: "0.78rem", fontWeight: "800", color: "var(--nox-text-muted)", marginBottom: "0.5rem" }}>{stat.icon}</div>
            <div style={{ fontSize: "2rem", fontWeight: "800", color: stat.color, lineHeight: 1 }}>{stat.value}</div>
            <div style={{ fontSize: "0.6875rem", color: "var(--nox-text-muted)", marginTop: "0.375rem", fontWeight: "600" }}>{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <div>
        <div className="nox-section-header">
          <span className="nox-section-title">Quick Actions</span>
          ...
          {actions.map((item) => (
            <a key={item.href} href={item.href} style={{ textDecoration: "none" }}>
              <div className="nox-card animate-fade-up" style={{
                ...
                <span style={{ fontSize: "0.82rem", minWidth: 58, color: "var(--nox-red-light)", fontWeight: 800 }}>{item.icon}</span>
                <div style={{ flex: 1 }}>
